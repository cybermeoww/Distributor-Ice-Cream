...
docRef.onSnapshot((doc) => {
    if (doc.exists) {
        ...
        if (pageTitle) pageTitle.textContent = `Manajemen Stok: ${data.namaProduk}`;
        updateStokDisplay();
    }
});
...
document.getElementById('card-masuk').addEventListener('click', async () => {
    const newValue = await showEditPopup('Ubah Stok Masuk', currentStokMasuk);
    if (newValue) docRef.update({ stokMasuk: parseInt(newValue) });
});
