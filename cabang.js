...
async function displayBranchData(selectedUserId) {
    ...
    ordersRef.where("userId", "==", selectedUserId).onSnapshot((pesananQuery) => {
        ...
        pesananQuery.forEach(doc => {
            ...
            // FILTER HANYA PRODUK YG DIPILIH
...
async function showEditPopup(title, inputValue) {
    const { value: newValue } = await Swal.fire({
        title: title, input: 'text', inputValue: inputValue,
...
document.getElementById('card-alamat').addEventListener('click', async (e) => {
    ...
    if (newValue) usersRef.doc(currentSelectedUserId).update({ alamat: newValue });
});
document.getElementById('card-status').addEventListener('click', async (e) => {
    ...
    const newValue = await showEditPopup('Ubah Status', currentStatus);
    if (newValue) usersRef.doc(currentSelectedUserId).update({ status: newValue });
});
